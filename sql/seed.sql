...
COMMENT ON COLUMN lantern.messages.ts IS 'autogenerated tsvector using language code in top 6 bits of message flags';

CREATE VIEW lantern.live_messages AS SELECT * FROM lantern.messages WHERE flags & MESSAGE_DELETED = 0;
...
-- Index specially for message searching, efficient limiting by rooms within a party and user in the same index
...
CREATE INDEX reactions_msg_idx              ON lantern.reactions        USING btree(msg_id);
...
SELECT
    rooms.*, party_members.user_id,
    CASE WHEN party_members.permissions1 = -1 THEN -1 ELSE
        COALESCE(allow1, 0) | (party_members.permissions1 & ~COALESCE(deny1, 0))
    END AS permissions1,
    CASE WHEN party_members.permissions2 = -1 THEN -1 ELSE
