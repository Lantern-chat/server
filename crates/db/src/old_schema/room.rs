...
impl Room {
    pub async fn insert(&self, client: &Client) -> Result<(), ClientError> {
        client.write
            .execute_cached(
                || "INSERT INTO lantern.rooms (id, party_id, name, topic, icon_id, sort_order, flags, parent_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
                &[&self.id, &self.party_id, &self.name, &self.topic, &self.icon_id, &self.sort_order, &self.flags.bits(), &self.parent_id],
            )
            .await?;
    ...
    pub async fn find(client: &Client, room_id: Snowflake) -> Result<Option<Room>, ClientError> {
        let row = client.read.query_opt_cached(
            || "SELECT party_id, name, topic, icon_id, sort_order, flags, parent_id FROM lantern.rooms WHERE id = $1",
            &[&room_id]
        ).await?;
    ...
    pub async fn of_party(client: &Client, party_id: Snowflake) -> Result<Vec<Room>, ClientError> {
        client.read.query_stream_cached(
            || "SELECT id, name, topic, icon_id, sort_order, flags, parent_id FROM lantern.rooms WHERE party_id = $1",
            &[&party_id]
        )
        .await?
        .map(|res| match res {
