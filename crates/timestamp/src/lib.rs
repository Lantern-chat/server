...
lazy_static::lazy_static! {
    static ref Timestamp_REGEX: Regex = RegexBuilder::new(r"
        ([0-9]{4})-?    # year
        ([0-9]{2})-?    # month
        ([0-9]{2})      # day
        T
    ...
    static ref Timestamp_REGEX2: Regex = Regex::new("([0-9]{4})-?([0-9]{2})-?([0-9]{2})T([0-9]{2}):?([0-9]{2}):?([0-9]{2})(\\.[0-9]+)?Z").unwrap();
}

pub fn parse_iso8061_regex(ts: &str) -> Option<PrimitiveDateTime> {
    let m = Timestamp_REGEX.captures(ts)?;
...
fn parse_iso8061(ts: &str) -> Option<PrimitiveDateTime> {
    let b = ts.as_bytes();
...
impl Timestamp {
    ...
    pub fn parse(ts: &str) -> Option<Self> {
        parse_iso8061(ts).map(Timestamp)
    }
}
            ...
            D: Deserializer<'de>,
        {
            ...
            impl<'de> Visitor<'de> for TsVisitor {
                ...
                fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                    formatter.write_str("an Timestamp Timestamp")
                }
