...
pub async fn get_room(state: State, auth: Authorization, room_id: Snowflake) -> Result<Room, Error> {
...
async fn get_room_simple(state: State, db: db::pool::Object, room_id: Snowflake) -> Result<Room, Error> {
    let row = db
