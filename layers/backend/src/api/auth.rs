...
pub async fn do_auth(state: &State, token: RawAuthToken) -> Result<Authorization, Error> {
...
pub async fn do_bot_auth(state: &State, token: SplitBotToken) -> Result<Option<Authorization>, Error> {
    unimplemented!()
}
