...
pub fn validate_username(config: &Config, username: &str) -> Result<(), Error> {
    if !config.shared.username_length.contains(&username.len()) || !USERNAME_REGEX.is_match(username) {
...
pub fn validate_password(config: &Config, password: &str) -> Result<(), Error> {
    if !config.shared.password_length.contains(&password.len()) || !PASSWORD_REGEX.is_match(password) {
