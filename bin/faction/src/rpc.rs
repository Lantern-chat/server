...
async fn encode_item<T, W, const N: usize>(mut out: W, item: &T) -> Result<(), Error>
where
    W: AsyncWrite + Unpin,
    T: Serialize<AllocSerializer<N>>,
{
    ...
    out.write_all(archived.as_slice()).await?;
...
where
    ...
    T: Serialize<AllocSerializer<N>>,
{
    let mut writer = AsyncFramedWriter::new(out);
    ...
    while let Some(item) = stream.next().await {
        ...
        let mut msg = writer.new_message();
    ...
    W: AsyncWrite + Unpin,
{
    ...
    macro_rules! c {
        ($first:ident$(::$frag:ident)+($($args:expr),*)) => {
            Box::pin(async move { encode_item(out, &crate::api::$first$(::$frag)+($($args),*).await?).await }).await
        };
    }
    macro_rules! s {
        ($first:ident$(::$frag:ident)+($($args:expr),*)) => {
            Box::pin(async move { encode_stream(out, crate::api::$first$(::$frag)+($($args),*).await?).await }).await
        };
    }
