...
pub fn messages(mut route: Route<ServerState>, auth: Authorization, room_id: RoomId) -> RouteResult {
    match route.next().method_segment() {
        ...
        (_, Exact(_)) => match route.param::<MessageId>() {
            Some(Ok(msg_id)) => match route.next().method_segment() {
                ...
                (&Method::PUT | &Method::DELETE, Exact("pin")) => match route.next().param::<FolderId>() {
